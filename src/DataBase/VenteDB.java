...
public class VenteDB {
	...
	public void addVente(int id, String date_heure, String nom_utilisateur, String prenom_utilisateur, String produit, Double tva, Double somme, String type) {
		 PreparedStatement stm;
		try {
			stm = DB.prepareStatement("INSERT INTO ventes (date_heure,nom_utilisateur,prenom_utilisateur,produit,tva,somme,status,modepaiement) Values( ?, ?, ?, ?, ?, ?,'valide',?)", Statement.RETURN_GENERATED_KEYS );
...
public void addProduitsV( String produit, Double quantité, Double prix, Double tva, Double tvaC, int numeroVente) {
	 PreparedStatement stm;
	try {
		stm = DB.prepareStatement("INSERT INTO produitscaisse (produit,prix,quantite,tva,numerovente) Values(?, ?, ?, ?,?)" );
		stm.setString(1, produit);
		stm.setDouble(2, prix);
		stm.setDouble(3, quantité);
		stm.setDouble(4, tva);
...
public ObservableList<Vente> loadProduitsV() {
	try {
		...
		vente.clear();
		while(Rs.next()) {
			vente.add(new Vente((Integer) Rs.getObject(1),  (String) Rs.getObject(2),(Double) Rs.getObject(3), (Integer) Rs.getObject(4), (Double) Rs.getObject(5), (Integer) Rs.getObject(6)));
		}
		Rs.close();
		stm.close();
		} catch (SQLException e) {
		...
		while(Rs.next()) {
			...
			button.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent arg0) {
					...
					int id = Integer.parseInt(arg0.getSource().toString().substring(44).replace("'", "").replace(" ", ""));
			...
			Analyse.add(new Analyse((Integer) Rs.getObject(1),  (String) Rs.getObject(2),(String) Rs.getObject(3), (String) Rs.getObject(4),button , (Double) Rs.getObject(6), (Double) Rs.getObject(7)));
		}
		Rs.close();
		stm.close();
		} catch (SQLException e) {
...
public ObservableList<Analyse> displayAllTickets() {
	try {
		PreparedStatement stm = DB.prepareStatement("select v.id,v.date_heure,sum(p.prix) as somme,v.produit,v.status from public.produitscaisse p join ventes v on p.numerovente=v.id where v.status='non valide' group by(v.id)  ");
		ResultSet Rs = stm.executeQuery();
		Analyse.clear();
		while(Rs.next()) {
			...
			Analyse.add(new Analyse( (String) Rs.getObject(2),(Double) Rs.getObject(3),button,button2));
		}
		Rs.close();
		stm.close();
		} catch (SQLException e) {
...
public ObservableList<Analyse> displayAllTicketsAttente(CaisseControl caisse, TableView<objects.Analyse> tabletickets) {
	try {
		...
		while(Rs.next()) {
			...
			button.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent arg0) {
					ProduitVenteControl produitsV = new ProduitVenteControl();
					int id = Integer.parseInt(arg0.getSource().toString().substring(41).replace("'", "").replace(":", ""));
					System.out.println("ID" +arg0.getSource().toString().substring(41).replace("'", "").replace(":", ""));
					produitsV.setProduitsV(loadProduitsVS(id));
		    		Stage primaryStage = new Stage();
					primaryStage.setScene(new Scene(produitsV));
					primaryStage.getIcons().add(new Image("/img/its_icoTR.png"));
			...
			button2.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent arg0) {
					...
					try {
						stm2 = DB.prepareStatement("Select produit,prix,quantite FROM produitscaisse WHERE numerovente="+idd);
						...
						while(Rs2.next())
						{
							...
							item.add(new Vente(0, (String)	Rs2.getObject(1),(Double) Rs2.getObject(2), (Integer) Rs2.getObject(3), produit.getTVA(), (produit.getTVA()*produit.getPrixvente()) / 100, 0));
			...
			Analyse.add(new Analyse( (String) Rs.getObject(2),(Double) Rs.getObject(3),button,button2,button3));
		}
		Rs.close();
		stm.close();
		} catch (SQLException e) {
	...
	public void UpdateTVA(int id, Double tva,String msg) {
		PreparedStatement stm;
		try {
			stm = DB.prepareStatement("UPDATE ventes SET tva=?,produit=?,modepaiement='cash' WHERE id=?" );
			stm.setDouble(1,tva);
	...
	public ObservableList<objects.Analyse> displayAllTicketsUtilisateur(String nom_utilisateur, String prenom_utilisateur) throws SQLException {
		...
		PreparedStatement stm = DB.prepareStatement("select v.id,sum(p.prix) as somme,v.produit,v.status \r\n" + 
				...
				"and v.nom_utilisateur=? and prenom_utilisateur=?  AND status='valide' \r\n" + 
				"group by(v.id) ");
		...
		while(Rs.next()) {
			...
			Analyse.add(new Analyse( (Double) Rs.getObject(2),button,button2));
		}
		Rs.close();
		stm.close();
	...
	public ObservableList<Analyse> displayAllTicketsFiltered(String datedeb, String datef, String name, String lastName) {
		...
		try {
			PreparedStatement stm = DB.prepareStatement("select v.id,v.date_heure,sum(p.prix) as somme,v.produit,v.status from public.produitscaisse p join ventes v on p.numerovente=v.id  where v.nom_utilisateur ="+ "'" +name +"'" + " AND v.prenom_utilisateur ="+ "'" +lastName +"'" + " AND SUBSTRING(v.date_heure FROM 1 FOR 10) >=? AND SUBSTRING(v.date_heure FROM 1 FOR 10) <= ? AND status='valide' group by(v.id) order by(date_heure);   ");
	...
	public Map<String,Long> getTopProduitFiltered(String datedeb,String datef)
	{
		...
		try {
			...
			PreparedStatement stm = DB.prepareStatement("select count(*) ,p.produit from public.produitscaisse p join ventes v on p.numerovente=v.id  where SUBSTRING(date_heure FROM 1 FOR 10) >=? AND SUBSTRING(date_heure FROM 1 FOR 10) <= ?  group by(p.produit)");
			stm.setString(1, datedeb);
			stm.setString(2, datef);
			...
			while (Rs.next() )
			{
				...
				String substringO;
					...
					res.put((String) Rs.getObject(2) ,(Long) Rs.getObject(1));
					System.out.println("nom produit2: " + (String) Rs.getObject(2));
	...
	public Map<String,Long> getTopProduitFilteredG(String datedeb,String datef)
	{
		...
		try {
			...
			while (Rs.next() )
			{
				System.out.println("size res = "+res.size());
				String substringO;
				substringO = (String) Rs.getObject(2);
				if(substringO.length() > 10) {
					res.put(substringO.substring(0, 10) ,(Long) Rs.getObject(1));
					System.out.println("nom produit: " + substringO.substring(10,substringO.length()));
				}
				else {
					res.put((String) Rs.getObject(2) ,(Long) Rs.getObject(1));
					System.out.println("nom produit2: " + (String) Rs.getObject(2));
	...
	public List<Analyse> loadAnalyseFiltred(String datef, String  datedeb) {
		...
		try {
			PreparedStatement stm = DB.prepareStatement("select * from ventes where SUBSTRING(date_heure FROM 1 FOR 10) >=? AND SUBSTRING(date_heure FROM 1 FOR 10) <= ? ORDER BY (date_heure)");
			stm.setString(1, datedeb);
			stm.setString(2, datef);
			...
			while(Rs.next()) {
				...
				AS.add(new Analyse((Integer) Rs.getObject(1),  (String) Rs.getObject(2),(String) Rs.getObject(3), (String) Rs.getObject(4),button , (Double) Rs.getObject(6), (Double) Rs.getObject(7)));
			}
			Rs.close();
			stm.close();
			} catch (SQLException e) {
